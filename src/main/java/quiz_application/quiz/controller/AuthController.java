...
public class AuthController {
     ...
     */
    private static final long OTP_EXPIRY_MILLIS = 300000;
    ...
    public String resendOtp(
        ...
        Integer resendCount =
                (Integer) session.getAttribute("resendCount");
    ...
    public String resetPasswordOtpPage(
            HttpSession session,
            Model model) {

        Long otpTime =
                (Long) session.getAttribute("resetOtpTime");

        if (otpTime != null) {

            long elapsed =
                    (System.currentTimeMillis() - otpTime) / 1000;

            long remaining =
                    Math.max(
                            (OTP_EXPIRY_MILLIS / 1000) - elapsed,
                            0);
    ...
    /**
     * Clear registration OTP session data.
     */
